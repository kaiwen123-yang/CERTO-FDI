...
SIGNAL_KEYS = ("q_meas", "qd_meas", "qdd_est", "tau_meas", "tau_nominal", "r_gmo", "qdd_true", "tau_cmd")
# Stage 2A addition: ``tau_cmd`` is the controller's *own* command (the noiseless version of
# ``tau_meas``), needed by the F1 actuator-efficiency and F6 command-path dictionaries. It is a
# deployment-available signal, never a truth-only one, and is *not* an encoder input -- the
# leakage scanner asserts that no model batch field named tau_cmd reaches an encoder
# (tests/test_stage2a_no_leakage.py).
...
class WindowSet:
    ...
    def batch(self, ids: Iterable[int], device: str | None = None) -> dict[str, torch.Tensor]:
        ...
        for i in ids:
            ...
            if want_cmd:
                out["tau_cmd"].append(self._sig["tau_cmd"][e][s : s + W])
