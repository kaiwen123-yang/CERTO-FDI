...
def known_issues_md(st: Stage, ev: dict, basis: dict, M: dict, tun: dict, r0: dict) -> str:
    ...
    lines = [f"# Known issues and limitations — Stage 1R-B ({st.layout.run_id})", "",
             ...
             "- Single pre-registered optimisation fix (before any final training): the first LiGRA-v2 head used the exact backward recursion without the invariant child-message gain that every PR #2 chain model (incl. chain_gnn_aug) has; healthy-validation-only tuning/diagnostics showed this hampers optimisation of every head (typed head lr<=1e-3: val loss plateau ~0.72-0.76 with early train/val divergence; free head 0.72 -> 0.34 with the gain; typed head at lr 2e-3, 25-epoch schedule 0.280 -> 0.218). The invariant gain g_i (49 params) was added, R0 re-run (PASS), and the 8-configuration tuning re-run; the pre-fix tuning runs are archived under c2_tuning/runs_v2_nogain_archive/ with a README. No fault data was involved and no transformation law changed.",
             "- Optimisation sensitivity: LiGRA-v2 needs the upper end of the shared learning-rate grid (2e-3) to leave an early memorising regime (train loss falls, healthy validation loss rises); the frozen 8-configuration grid covers this and the selection is by healthy validation RMSE only.",
