...
def _aursad() -> DatasetD0:
    card = FeasibilityCard(
        ...
        version="1.1 (concept DOI 10.5281/zenodo.4487072; v1.0 = record 4487073)",
        ...
        label_semantics="0 normal (1420), 1 damaged screw (221), 2 extra assembly component (183), 3 missing screw (218), 4 damaged thread (3); label 5 = 2049 supplementary loosening/picking samples",
    ...
    return DatasetD0(
        ...
        status="BLOCKED_PENDING_SCHEMA_AUDIT",
        ...
        signal_groups=[],
        split_definition="TO_BE_DETERMINED from the official loader; must be episode-safe at sample granularity.",
        ...
        open_verifications=[
            "The documented feature count (registry says 134) is not stated in the README and must be read from the HDF5 schema after download.",
            "Whether UR3e joint positions/velocities/currents/moments are present is UNVERIFIED; the physics grade stays TO_BE_AUDITED until the schema is read.",
            "arXiv abstract says 2042 samples, the official README says 2045; the authoritative count must come from the file.",
        ],
        allowed_claims=["process anomaly detection on a real UR3e screwdriving cell"],
        forbidden_claims=[
