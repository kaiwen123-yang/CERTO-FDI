...
def test_masked_delta_tau_matches_explicit_recursion(rng):
    ...
    from certo_fdi.models.ligra_chain import backward_recursion, build_model, run_front_end
    ...
    n = chain.n_links
    for name in ("ligra_v2_typed", "chain_gnn_aug", "ligra_free_output"):
        m = build_model(name, tc, 6, {"scalar_hidden_dim": 16, "scalar_layers": 1, "ligra_v2": {"scalar_hidden": 8, "twist_channels": 2, "wrench_channels": 2}}).to(dtype)
