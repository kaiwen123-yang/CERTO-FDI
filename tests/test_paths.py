...
from certo_fdi.paths import RUN_SUBDIRECTORIES, STAGE, create_or_resume_run, require_external_path
...
def test_run_layout_created_and_resumable(tmp_path: Path):
    ...
    assert (tmp_path / "04_runs" / STAGE / "LATEST_RUN.txt").read_text().strip() == str(layout.run_root)
